...
void hugepage_util_init(void)
{
	...
	sprintf(cmd,"rm -f %s/vecring*",HUGEPAGE_PATH);
	system(cmd);
}
static uint64_t preserve_vm_area(int nr_pages,void **raw_addr)
{
        void* addr=NULL;
        int fd;
...
{/*vring_id is a string which identifys which vector ring the hugefile belongs to*/
	...
	*nr_hp_files=0;
	for(idx=0;idx<max_huge_nr;idx++){
		...
		sprintf(hp_files[idx].path,"%s/vecring-%s-%d",HUGEPAGE_PATH,vring_id,idx);
...
int allocate_mmeory_from_hugepage_fs(struct hugepage_memory *hp_mem,char *vring_id,int length,int prefered_node)
{
	int rc;
	rc=map_all_hugepages_original(vring_id,
		length,
		prefered_node,
		&hp_mem->hp_files,
		&hp_mem->nr_files);
...
int hugepage_util_module_test(void)
{
	...
	hugepage_util_init();
	int rc=allocate_mmeory_from_hugepage_fs(&hp_mem,"foo",1024*1024*2*25+1,0);
	printf("rc:%d %p\n",rc,(void*)hp_mem.base);
	#if 0
